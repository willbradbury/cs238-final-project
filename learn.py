...
class Learner(object):
  ...
  def beta(self, state):
    if self.local_approx:
      ...
      weights = 1 / np.sum((translated_state - idx)**2, axis=1)
  ...
  def update_step(self, s_t, a_t, idx_t, weights_t):
    ...
    # print self.theta_index(s_t, a_t, idx_t)
    # print self.N[self.theta_index(s_t, a_t, idx_t)]
    # print weights_t
    self.N[self.theta_index(s_t, a_t, idx_t)] += weights_t
    # print self.N[self.theta_index(s_t, a_t, idx_t)]
    # print np.count_nonzero(self.N)
  ...
  def train(self, n_epochs):
    ...
    for epoch in xrange(n_epochs):
      ...
      while not self.simulator.is_finished():
        ...
        s_t, a_t = self.update_step(s_t, a_t, idx_t, weights_t)
